...
export function MobileMenu({
  ...
  const links = [
    ...navigation.map((item) => ({ href: `/category/${item.slug}`, label: item.name, id: item.id })),
    { href: '/custom-orders', label: 'Custom jerseys', id: 'custom' },
    { href: '/products', label: 'All products', id: 'products' },
          ...
          <div className="px-4 py-4">
            ...
            >
              {links.map((item) => (
                ...
                >
                  <Link href={item.href} className="block py-2 font-heading text-lg uppercase" onClick={onClose}>
                    {item.label}
                  </Link>
                </motion.div>
              ))}
