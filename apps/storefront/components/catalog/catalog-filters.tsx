...
export function CatalogFilters({ facets }: { facets: StorefrontCatalogFacets }): React.JSX.Element {
  ...
  return (
    <form className="grid gap-4 border border-border p-4 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-7" onSubmit={(event) => event.preventDefault()}>
      <label className="grid gap-1 text-xs font-semibold uppercase tracking-wider md:col-span-2">
        Search
        <Input
