...
async function main(): Promise<void> {
  ...
  for (const category of categoryTree) {
    ...
    const record = await prisma.category.upsert({
      ...
      update: { name: category.name, parentId: parentId ?? null, status: 'ACTIVE', image: DEMO_ASSET(`categories/${slug}.jpg`) },
      create: {
        tenantId: tenant.id,
        ...
        image: DEMO_ASSET(`categories/${slug}.jpg`),
  ...
  for (const [productIndex, product] of products.entries()) {
    ...
    await prisma.productImage.create({
      data: {
        tenantId: tenant.id,
        productId: record.id,
        url: DEMO_ASSET(`products/${slug}-front.jpg`),
        storageKey: `demo/products/${slug}-front.jpg`,
    ...
    await prisma.productImage.create({
      data: {
        tenantId: tenant.id,
        productId: record.id,
        url: DEMO_ASSET(`products/${slug}-back.jpg`),
        storageKey: `demo/products/${slug}-back.jpg`,
