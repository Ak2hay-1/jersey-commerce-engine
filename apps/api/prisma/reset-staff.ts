/**
 * Reset staff accounts for a tenant: keep only Super Admin + Owner, remove all other staff.
 *
 * Usage (local):
 *   TENANT_SLUG=jerzyfy STAFF_PASSWORD='YourPassword1!' npx ts-node --compiler-options '{"module":"CommonJS"}' prisma/reset-staff.ts
 *
 * Usage (production VM):
 *   bash infra/docker/reset-staff.sh jerzyfy
 */
...
const TENANT_SLUG = process.env.TENANT_SLUG?.trim() || 'jerzyfy';
const STAFF_PASSWORD = process.env.STAFF_PASSWORD?.trim() || 'DevPassword123!';

const STAFF: Record<'SUPER_ADMIN' | 'OWNER', { email: string; name: string }> = {
  SUPER_ADMIN: {
...
const STAFF_ROLE_CODES: Array<'SUPER_ADMIN' | 'OWNER'> = ['SUPER_ADMIN', 'OWNER'];

async function reassignUserReferences(tenantId: string, from: string, to: string): Promise<void> {
...
async function removeStaffUser(tenantId: string, userId: string, reassignToUserId: string): Promise<void> {
