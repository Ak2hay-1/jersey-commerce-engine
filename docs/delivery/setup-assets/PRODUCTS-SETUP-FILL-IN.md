...
### Fixing products that already have the wrong size

If a live product shows all sizes in one button (SKU like `…-SMLXLXXL`):

1. Open **ERP → Products →** that product → Variants
2. Archive or remove the compound-size row
3. Click **Add adult sizes** (or add S, M, L, XL, XXL as separate rows)
4. Set Cost, Selling, and Compare-at on each row; save
5. Re-enter opening stock per size (§4)
6. Confirm the PDP shows separate size buttons and (if Compare-at is set) selling + MRP + `% off`
...
For each product: create product → add variants (**one size per row**; cost + selling required; Compare-at optional; SKU auto) → upload Front/Side/Back images → set status ACTIVE → featured if needed → then §4 opening stock.
