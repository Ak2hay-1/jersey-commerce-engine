...
This phase does **not** include the website CMS admin UI (Phase 13), POS UI, ERP screens, live payment capture, reviews, or wishlists.
...
The storefront never sends `tenantId`. Prices, tax, discounts, and stock are never trusted from the client. Checkout re-quotes from live catalog data.
...
Homepage sections are stored on `WebsiteSettings.homepageConfig.sections`. Disabled sections are omitted. Missing config falls back to a default section list. A later CMS phase will edit this JSON; this phase only consumes it.
